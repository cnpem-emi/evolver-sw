...
def process_old_directory(directory):
    ...
    temp_cal_raw_directory = os.path.join(
        directory, RAW_CAL_DIRECTORY, TEMP_CAL_DIRECTORY
    )
    od_cal_fit_directory = os.path.join(
        directory, FITTED_CAL_DIRECTORY, OD_CAL_DIRECTORY
    )
    temp_cal_fit_directory = os.path.join(
        directory, FITTED_CAL_DIRECTORY, TEMP_CAL_DIRECTORY
    )

    if not os.path.exists(raw_cal_directory) or not os.path.exists(
        fitted_cal_directory
    ):
    ...
    cals = get_cal_list(
        raw_od_calibrations, measured_od_datas, od_fit_calibrations, "od"
    )
    cals = cals + get_cal_list(
        raw_temp_calibrations, measured_temp_datas, temp_fit_calibrations, "temperature"
    )
            ...
            for step in vial_datas:
                for param, param_vial_datas in step.items():
                    if param in VALID_PARAMS:
                        ...
                        raw_calibrations[filename_split].append(
                            create_raw(param, filename_split, cal_datas[param])
                        )
...
def process_fit(calibration_fit_directory, fit_type, params):
    ...
    for filename in os.listdir(calibration_fit_directory):
        fit = process_old_fit(
            os.path.join(calibration_fit_directory, filename),
            filename,
            fit_type,
            active,
            params,
    ...
    for filename in raw_calibrations.keys():
        try:
            new_cal = {
                ...
                "measuredData": measured_datas[filename],
                "raw": raw_calibrations[filename],
                "fits": fit_calibrations[filename],
            }
            cals.append(new_cal)
        except KeyError:
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        action="store",
        dest="directory",
        help="Location of old calibration directory",
    )
    parser.add_option(
        "-f",
        "--file",
        action="store",
        dest="filename",
        help="Filename of new calibration file",
    )
